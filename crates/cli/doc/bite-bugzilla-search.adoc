...
+
.Create a template:
 bite -c gentoo search -c 1d -n --to template.toml
...
All user-related options search for matching substrings by default. Multiple
values can be specified in a comma-separated list for logical AND or multiple
...
+
.With CC:
 bite -c gentoo search --cc
+
.Without CC:
 bite -c gentoo search --cc false
+
...
+
.With QA contact:
 bite -c gentoo search --qa
+
.Without QA contact:
 bite -c gentoo search --qa false
+
See the information above in the <<User Options>> section for regular values.
