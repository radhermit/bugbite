...
*--attachment-creator* <value[,...]>::
    Restrict by attachment creator.
+
...
.Attachments created by user:
 bite bugzilla search --attachment-creator user
...
Various restrictions can use match operators to alter a value's query application.
Values with no or unknown operators default to case-sensitive substring matching.
