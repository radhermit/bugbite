...
+
.Create a template:
 bite bugzilla search -c 1d -n --to template.toml
...
formatted output:
+
.Return all open bugs created in the past day rendered in JSON:
...
for the search. For example, given a search that would return 100 bugs, setting
limit to 10 and offset to 10 would return bugs 11 through 20 from the set
of 100.
...
interface of the form `cf_field_name` with the `cf_` prefix being optional.
+
.Custom field containing `test`:
 bite bugzilla search --cf field_name test
+
.Custom field equals `test`:
 bite bugzilla search --cf field_name '== test'
