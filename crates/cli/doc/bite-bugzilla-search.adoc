...
section for more information.
+
.Search for comments tagged as spam:
 bite -c gentoo search --comment-tag spam
...
Operators are applied to values in prefixes with a single space between,
wrapping the entire string in quotes. Note that operators using `!` will
require using single quotes in shell environments that perform command
expansion. See the following examples for proper usage:
