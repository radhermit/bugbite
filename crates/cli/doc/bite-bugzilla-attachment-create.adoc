...
.Attach build.log with forced MIME type:
 bite bugzilla -c gentoo attachment create 10 build.log --mime text/plain
...
.Compress attachment using the default type:
 bite bugzilla -c gentoo attachment create 10 file --compress
+
.Compress attachment using zstd:
 bite bugzilla -c gentoo attachment create 10 file --compress zstd
+
...
.Auto-compress using the defaults:
 bite bugzilla -c gentoo attachment create 10 file --auto-compress
+
.Auto-compress using zstd with 5MB size limit:
 bite bugzilla -c gentoo attachment create 10 file --auto-compress 5 --compress zstd
...
.Auto-truncate to 1000 lines:
 bite bugzilla -c gentoo attachment create 10 file.txt --auto-truncate
+
.Auto-truncate to 5000 lines and compress using zstd:
 bite bugzilla -c gentoo attachment create 10 file.txt --auto-truncate 5000 --compress zstd
...
.Attach to all matching bugs:
 bite bugzilla -c gentoo search bugbite -f id | bite bugzilla -c gentoo attachment create - path/to/file

.Attach to multiple bugs:
 bite bugzilla -c gentoo attachment create 3,4,5 file
...
.Attach multiple files:
 bite bugzilla -c gentoo attachment create 3 file1 file2 path/to/file3

== Examples

.Attach compressed tarball of directory:
 bite bugzilla -c gentoo attachment create 10 path/to/dir

.Attach tarball compressed with zstd:
 bite bugzilla -c gentoo attachment create 10 path/to/dir --compress zstd
