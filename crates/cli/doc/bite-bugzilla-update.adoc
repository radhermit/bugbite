...
.Update bug using template:
 bite bugzilla update 123 --from path/to/new.toml -c 'updated using template'

.Create template using specified values:
 bite bugzilla update -p TestProduct -C TestComponent --to path/to/new.toml --dry-run
...
+
.Update custom field:
 bite bugzilla update 10 --cf stabilisation_atoms "sys-libs/pkgcraft arm64"
