...
+
.Add alias `a1`:
 bite -c gentoo modify 10 --alias +a1
+
.Add alias `a2` and remove `a1`:
 bite -c gentoo modify 10 --alias +a2,-a1
+
.Set aliases to `a3`:
 bite -c gentoo modify 10 --alias a3
...
+
.Assign to yourself:
 bite -c gentoo modify 123 --assignee @me
+
.Reset assignee to the default value:
 bite -c gentoo modify 123 --assignee
...
.Add blocker on bug 1:
 bite -c gentoo modify 10 --blocks +1
+
.Add bug 2 and remove bug 1 blockers:
 bite -c gentoo modify 10 --blocks +2,-1
+
.Set blockers to bug 3:
 bite -c gentoo modify 10 --blocks 3
...
bite -c gentoo modify 10 --cc=+test1@email.com,-test2@email.com
...
.Create a comment from a file:
 bite -c gentoo modify 10 --comment-from path/to/file.txt

*-P, --comment-is-private*::
    Mark created comment as private.
+
.Create a private comment:
 bite -c gentoo modify 10 --comment test --comment-is-private
+
.Create a private comment from a file:
 bite -c gentoo modify 10 --comment-from path/to/file.txt --comment-is-private
+
.Create a private reply to last comment:
 bite -c gentoo modify 10 --reply --comment-is-private
...
.Toggle comment 1 privacy:
 bite -c gentoo modify 10 --comment-privacy 1
+
.Toggle comment 1 and 2 privacy:
 bite -c gentoo modify 10 --comment-privacy 1,2
+
.Toggle all comment privacy:
 bite -c gentoo modify 10 --comment-privacy ..
+
.Disable comment 1 and 2 privacy:
 bite -c gentoo modify 10 --comment-privacy 1,2:false
+
.Mark comments 2-5 private:
 bite -c gentoo modify 10 --comment-privacy 2..=5:true
...
.Add dependency on bug 1:
 bite -c gentoo modify 10 --depends +1
+
.Add bug 2 and remove bug 1 dependencies:
 bite -c gentoo modify 10 --depends +2,-1
+
.Set dependencies to bug 3:
 bite -c gentoo modify 10 --depends 3
...
+
.Add `test?` flag:
 bite -c gentoo modify 10 --flags "test?"
+
.Add `check+` and remove `test?` flags:
 bite -c gentoo modify 10 --flags check+,testX
...
+
.Add to `admin` group:
 bite -c gentoo modify 10 --groups +admin
+
.Add `test` and remove `admin` groups:
 bite -c gentoo modify 10 --groups +test,-admin
...
+
.Add `key` keyword:
 bite -c gentoo modify 10 --keywords +key
+
.Add `test` and remove `key` keywords:
 bite -c gentoo modify 10 --keywords +test,-key
+
.Set keywords to `verify`:
 bite -c gentoo modify 10 --keywords verify
...
+
.Assign to yourself:
 bite -c gentoo modify 123 --qa @me
+
.Reset to default:
 bite -c gentoo modify 123 --qa
...
+
.Add URL to bug 2:
 bite -c gentoo modify 10 --see-also 2
+
.Add bug 3 and remove bug 2 URLs:
 bite -c gentoo modify 10 --see-also=+3,-2
+
.Add URL to external bug:
 bite -c gentoo modify 10 --see-also https://url/to/bug/5
